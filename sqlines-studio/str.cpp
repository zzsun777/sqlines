...
const char* Str::GetCopy(const char *input, int size)
{
	if(input == NULL)
		return NULL;
...
bool Str::CompareNumberAsString(const char *first, int flen, const char *second, int slen)
{
	...
	if(len1 != 0 && len2 != 0 && memcmp(part1, part2, len1) != 0)
		return false;
	...
	GetFractionPartLen(cur1, flen - (cur1 - first), &len1);
	GetFractionPartLen(cur2, slen - (cur2 - second), &len2);
	...
	if(len1 != 0 && len2 != 0 && memcmp(cur1, cur2, len1) != 0)
		return false;
...
void Str::TrimToAbbr(std::string &input, std::string &output, int max_len)
{
	...
	while(*cur)
	{
		...
		{
			int c = strncmp(abbr[i].word, cur, abbr[i].len);
